...
export function MotionPreviewCard({ preview }: MotionPreviewCardProps) {
  ...
  const motionId = preview.id.replace(/_in_place\.glb$/, "");

  return (
    <Link
      className="block rounded-lg focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-zinc-950"
      href={`/motions/${motionId}`}
    >
      <article className="grid h-[320px] grid-rows-[220px_minmax(0,1fr)] overflow-hidden rounded-lg border border-zinc-200 bg-transparent shadow-sm transition-shadow hover:shadow-md">
